...
;; if there is .uncrustify.cfg in current directory, then use it
;; or search the git/svn root directory, if found .uncrustify.cfg, then use it
;; last use ~/.uncrustify.cfg
(defun get-uncrustify-cfg-file ()
  (if (file-exists-p (concat default-directory ".uncrustify.cfg"))
	(concat default-directory ".uncrustify.cfg")
	(if (vc-root-dir)
	  (if (file-exists-p (concat (vc-root-dir) ".uncrustify.cfg"))
		(concat (vc-root-dir) ".uncrustify.cfg")
  ...
  (if lang
      (let ((start (or start-in (point-min)))
            ...
            (original-line (uncrustify-point->line (point)))
			 (cmd (concat uncrustify-bin " -c " (get-uncrustify-cfg-file)
					" -l " lang))
...
;; uncrustify selected region
(defun uncrustify-region ()
  (interactive)
  (save-restriction
    (widen)
    (uncrustify-invoke-command
	  (uncrustify-get-lang-from-mode)
	  (region-beginning) 
	  (region-end) 
	  ))) 
